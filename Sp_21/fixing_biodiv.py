...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 12 22:41:27 2021
...
data = pandas.read_csv('mos_processed.csv', low_memory = False)
...
dups = data.pivot_table(index=['siteID', 'year', 'concated_column'], columns= 'taxonID', values='individualCount', aggfunc=len)
...
df2 = data.groupby(['siteID', 'year', 'concated_column', "taxonID" ],as_index=False).agg({'individualCount' : "sum"})

##Now to check that that actually happened. All the values here should be 1

No_dups = df2.pivot_table(index=['siteID', 'year', 'concated_column'], columns= 'taxonID', values='individualCount', aggfunc=len)
...
biodiv_data = df2.pivot(index=['siteID', 'year', 'concated_column'], columns= 'taxonID', values='individualCount')
...
rich_mean = mergedDf.groupby(['year', 'siteID']).agg({'richness': ['mean', 'min', 'max']})

print(rich_mean)

shannon_mean = mergedDf.groupby(['year', 'siteID']).agg({'shannon': ['mean', 'min', 'max']})
...
stats.f_oneway(mergedDf["richness"][mergedDf['siteID'] == 'DSNY'],
               mergedDf["richness"][mergedDf['siteID'] == 'JERC'],
               mergedDf["richness"][mergedDf['siteID'] == 'OSBS'],
              mergedDf["richness"][mergedDf['siteID'] == 'MLBS'])


from statsmodels.stats.multicomp import pairwise_tukeyhsd
tukey = pairwise_tukeyhsd(endog=mergedDf["richness"],groups = mergedDf['siteID'], alpha=0.05)
print(tukey)
